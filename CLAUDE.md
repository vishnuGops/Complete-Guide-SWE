...
- **Design** follows `docs/DESIGN.md`, and its section 10 checklist is part of every UI review. In short: semantic tokens only (never a ramp colour or raw hex), one accent, 8-pt spacing, Inter + JetBrains Mono, no gradients, no hero sections, no emoji in UI chrome, no card grids with drop shadows, real keyboard support, both themes reviewed. Components are built with the screen that needs them, not ahead of time; the four primitives in `apps/web/src/ui/` are the exception and the reference page is `/dev/kitchen-sink`. Contrast is enforced by `apps/web/src/styles/contrast.test.ts`, which measures the tokens rather than trusting them. If a screen looks like a generic dashboard template, it is wrong.
