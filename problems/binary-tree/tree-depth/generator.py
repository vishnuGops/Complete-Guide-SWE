...
def generate(rng: random.Random) -> Iterator[Dict[str, Any]]:
    ...
    for n in (3, 7, 12, 40, 150):
        yield _case(_encode(_random_tree(rng, n, -100, 100)))

    for depth in (3, 5, 8):
        yield _case(_encode(_full(rng, depth, -1000, 1000)))

    yield _case(_encode(_chain(rng, 800, -10, 10, left=True)), "a long left chain")
    yield _case(_encode(_chain(rng, 800, -10, 10, left=False)), "a long right chain")

    yield _case(_encode(_random_tree(rng, 2000, -(10**9), 10**9)), "the stated maximum")
    yield _case(_encode(_chain(rng, 2000, -(10**9), 10**9)), "the stated maximum as a chain")
