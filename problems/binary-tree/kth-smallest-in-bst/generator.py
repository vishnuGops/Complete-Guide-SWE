...
def _case(values: List[Any], k: int, name: str = None) -> Dict[str, Any]:
...
def generate(rng: random.Random) -> Iterator[Dict[str, Any]]:
    yield _case([5], 1, "a single node")
    yield _case([3, 1, 4, None, 2], 1, "the smallest")
    yield _case([3, 1, 4, None, 2], 3, "the third smallest")
    yield _case([3, 1, 4, None, 2], 4, "the largest")
    yield _case([-(10**9), None, 10**9], 2, "the extremes of the stated range")

    for n in (2, 5, 20, 90):
        values = _distinct(rng, n, 10**4)
        tree = _bst(rng, values)
        yield _case(_encode(tree), 1)
        yield _case(_encode(tree), n)
        yield _case(_encode(tree), rng.randint(1, n))

    # A sorted insertion, so the tree is a right chain and k = 1 is at the top.
    ascending = sorted(_distinct(rng, 600, 10**6))
    yield _case(_encode(_bst(rng, ascending)), 1, "a right chain, smallest at the root")
    yield _case(_encode(_bst(rng, ascending)), 600, "a right chain, largest at the bottom")

    big = sorted(_distinct(rng, 2000, 10**9))
    yield _case(_encode(_balanced_bst(big)), 1, "the stated maximum, k = 1")
    yield _case(_encode(_balanced_bst(big)), 2000, "the stated maximum, k = n")
    yield _case(_encode(_balanced_bst(big)), 997, "the stated maximum, k in the middle")
