...
def generate(rng: random.Random) -> Iterator[Dict[str, Any]]:
    ...
    yield _case([1, 2], "a tie, broken towards the root")
    yield _case([1, 2, 3], "the second level is wider")
    yield _case([1, 2, 3, 4, 5, None, 6], "the bottom level is the widest")
    yield _case([-(10**9), 10**9], "the extremes of the stated range")
    yield _case(_encode(_chain(rng, 10, -5, 5)), "a chain, every level a tie at one")
