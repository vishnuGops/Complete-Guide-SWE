...
def generate(rng: random.Random) -> Iterator[Dict[str, Any]]:
    ...
    yield _case([1, 2], "a left child only")
    yield _case([1, None, 2], "a right child only, already a chain")
    yield _case([1, 2, 5, 3, 4, None, 6], "a tree with both subtrees")
    yield _case([-(10**9), 10**9], "the extremes of the stated range")
    yield _case(_encode(_chain(rng, 8, -5, 5, left=True)), "a left chain, which reverses direction")
    yield _case(_encode(_chain(rng, 8, -5, 5, left=False)), "a right chain, already flat")
